...
void Render::setViewport() {
    ...
    depths = new number_t[width * height];
    cudaMalloc((void**)&gpu_colors, sizeof(Vector4) * width * height);
    cudaMalloc((void**)&gpu_depths, sizeof(number_t) * width * height);
}
...
struct RenderContext {
    ...
    inline __device__ number_t intersect_real_bbox(const Vector& p, const Vector& direction) {
        Vector bmin = bbox_real_min;
        Vector bmax = bbox_real_max;
        // if inside, no intersection.
        if(p <= bmax && p >= bmin) return 0;
        // Test for x.
        if(p.x < bmin.x && direction.x > 0) {
    ...
    inline __device__ bool find_zero_crossing(const Vector& p, const Vector& direction, number_t& t_result, Vector& normal) {
        ...
        number_t t = intersect_real_bbox(p, direction);
        if(t < 0) return false;
        //if(t_real > t) t = t_real;
        t = t + 1e-5;
            ...
            if(pos <= bmax && pos >= bmin) {
                ...
                if(val * val_prev <= 0) {
                    ...
                    if(val != 0) {
                        ...
                        if(pos >= bbox_real_min && pos <= bbox_real_max) return true;
                        return false;
                    }
                }
                val_prev = val;
...
__device__ inline Vector4 ray_tracing(RenderContext& ctx, const Vector& p, const Vector& direction, float& depth) {
    number_t t;
    Vector normal;
...
__global__ void compute_render(RenderContext ctx) {
    ...
    // Initial light.
    ctx.gpu_colors[idx] = ray_tracing(ctx, ctx.eye, direction, ctx.depths[idx]);
}
...
void Render::render() {
    ...
    ctx.bbox_real_min = bbox_real_min;
    ctx.bbox_real_max = bbox_real_max;
    ctx.gpu_colors = gpu_colors;
    ctx.depths = gpu_depths;
    ...
    ctx.surface_threshold = fs.resting_density / 3.0f;
    ...
    cudaMemcpy(depths, gpu_depths, sizeof(number_t) * width * height, cudaMemcpyDeviceToHost);
    gpuErrchkCritical(cudaGetLastError());
}
